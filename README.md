...
Windows host CMake needs **`rc.exe`** (Windows SDK) or **`llvm-rc`** (LLVM). That is `CMAKE_RC_COMPILER`. Do not install MinGW `windres`. `scripts/build_host.py` passes a quoted, forward-slash path (`"C:/Program Files (x86)/.../rc.exe"`) so `\P` in Program Files is not an invalid escape.
