...
Host and firmware Python builds write the same kind of shareable log (gitignored `local/`):

| Command | Log |
| --- | --- |
| `python -u scripts\build_host.py` | `local\build_host-last.log` |
| `python -u scripts\run_coverage.py` | `local\run_coverage-last.log` |
| `python -u scripts\build_lcc_wifi.py` | `local\build_lcc_wifi-last.log` |
...
Windows host CMake needs **`rc.exe`** (Windows SDK) or **`llvm-rc`** (LLVM). That is `CMAKE_RC_COMPILER`. Do not install MinGW `windres`. `scripts/build_host.py` now passes `-DCMAKE_RC_COMPILER=` when it finds one.
